...
public class RegisterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonRegister.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                if (!(confirmEmail(email) && confirmHeight(height) && confirmWeight(weight) && confirmPassword(password))) {
                    return;
                }
        ...
        textViewLogin = findViewById(R.id.register_button_have_account);
    ...
    private boolean confirmPassword(String password) {
        if (password.equals("")) {
            ...
            textInputLayoutEmail.setError("Please enter password at least 6 characters long");
            return false;
        }
        return true;
    }
    ...
    private boolean confirmHeight(String height) {
        if (height.equals("")) {
            textInputLayoutEmail.setError("Enter your height");
            return false;
        } else if (Integer.parseInt(height) < 40) {
            textInputLayoutEmail.setError("Height must be > 40");
            return false;
        } else if (Integer.parseInt(height) > 230) {
            textInputLayoutEmail.setError("Height must be < 230");
            return false;
        }
        return true;
    }
    ...
    private boolean confirmWeight(String weight) {
        if (weight.equals("")) {
            ...
            textInputLayoutEmail.setError("Weight must be > 30");
            return false;
        } else if (Integer.parseInt(weight) > 300) {
            textInputLayoutEmail.setError("Weight must be < 300");
            return false;
        }
        return true;
    }
